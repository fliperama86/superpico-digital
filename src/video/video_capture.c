...
static const uint16_t g_pixel_lut[4] = {
...
void video_capture_init(uint32_t height) {
    ...
    dma_channel_configure(g_dma_chan, &dc, g_line_buffers[0], &g_pio_snes->rxf[g_sm_pixel], SNES_H_ACTIVE, false);
}

void video_capture_run(void) {
    while (1) {
        // 1. Detect VSync Falling Edge (Active Video Start) in C
        while (!gpio_get(PIN_SNES_VBLANK)) tight_loop_contents(); // Wait for Blanking
        while (gpio_get(PIN_SNES_VBLANK)) tight_loop_contents();  // Wait for Active Video
        ...
        for (uint16_t y = 0; y < g_snes_height; y++) {
            ...
            if (y + 1 < g_snes_height) {
                dma_channel_set_trans_count(g_dma_chan, SNES_H_ACTIVE, false);
                dma_channel_set_write_addr(g_dma_chan, g_line_buffers[(y + 1) % 2], true);
            }
